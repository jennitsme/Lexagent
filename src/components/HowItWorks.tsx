...
function HexagonIcon({ Icon, index }: { Icon: typeof FileText; index: number }) {
  return (
    <div className="relative w-16 h-16 flex items-center justify-center">
      ...
      <Icon className="w-6 h-6 text-slate-700 relative z-10" strokeWidth={1.5} />
    </div>
  );
}
...
function StepConnector({ isLast }: { isLast: boolean }) {
  ...
  return (
    <div className="hidden md:flex flex-col items-center py-2">
      ...
      <svg width="12" height="12" viewBox="0 0 12 12" className="text-slate-700/20 mt-1">
    ...
    <section id="transfer" className="py-32 px-6 bg-white relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        >
          ...
          <h2 className="text-5xl md:text-7xl font-black tracking-tighter text-slate-700 mb-4">
            HOW IT WORKS
          </h2>
            ...
            >
              ...
              >
                ...
                <div className="flex items-start justify-between mb-5 relative z-10">
                  ...
                  <div className="text-right">
                    ...
                    <span className="font-mono text-2xl font-black text-slate-700/10 group-hover:text-slate-700/20 transition-colors">{step.id}</span>
                  </div>
                </div>

                <div className="relative z-10 flex-1">
                  <h3 className="text-xl font-black tracking-tight text-slate-700 mb-1 group-hover:tracking-normal transition-all duration-300">
                    {step.title}
                  </h3>
                  ...
                  <div className="flex items-center justify-between">
                    {step.metrics.map((metric, mi) => (
                      <div key={mi} className="text-left">
                        ...
                        <span className="font-mono text-xs font-bold text-slate-700/70 group-hover:text-slate-700 transition-colors">{metric.value}</span>
                      </div>
                    ))}
                  </div>
                ...
                <svg className="absolute bottom-0 right-0 w-24 h-24 text-slate-700/[0.02] group-hover:text-slate-700/[0.05] transition-colors duration-500" viewBox="0 0 100 100">
              ...
              {index < steps.length - 1 && (
                <div className="hidden lg:flex absolute -right-2 top-1/2 -translate-y-1/2 z-20 items-center">
                  <svg width="16" height="16" viewBox="0 0 16 16" className="text-slate-700/20">
          ...
          <div className="inline-flex items-center gap-6 px-8 py-4 border border-sky-200/65 bg-sky-50/70/50">
            ...
            {[
              ...
              <div key={i} className="text-center px-4">
                ...
                <span className="font-mono text-sm font-bold text-slate-700">{stat.value}</span>
              </div>
            ))}
          </div>
        </motion.div>
