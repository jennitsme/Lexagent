...
function HexagonIcon({ Icon, index }: { Icon: typeof FileText; index: number }) {
  return (
    <div className="relative w-16 h-16 flex items-center justify-center">
      ...
      <Icon className="w-6 h-6 text-blue-700 relative z-10" strokeWidth={1.5} />
    </div>
  );
}
  ...
  return (
    <div className="hidden md:flex flex-col items-center py-2">
      <div className="w-[1px] h-16 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-b from-blue-300/70 via-sky-300/40 to-transparent" />
        <div
          className="absolute top-0 left-0 w-full h-4 bg-gradient-to-b from-blue-500/70 to-transparent"
          style={{ animation: 'scan-line 2s linear infinite' }}
        />
      </div>
      <svg width="12" height="12" viewBox="0 0 12 12" className="text-blue-700/20 mt-1">
    ...
    <section id="transfer" className="py-32 px-6 bg-white relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        >
          ...
          <h2 className="text-5xl md:text-7xl font-black tracking-tighter text-blue-700 mb-4">
            HOW IT WORKS
          </h2>
          ...
          {steps.map((step, index) => (
            ...
            >
              <motion.div
                ...
                className="relative bg-white border border-sky-200/70 p-6 h-full flex flex-col overflow-hidden transition-shadow duration-500 hover:shadow-[0_8px_40px_rgba(0,0,0,0.08)] hover:border-sky-300/80"
              >
                <div className="absolute top-0 left-0 w-3 h-3 border-t border-l border-sky-300/80 transition-all duration-300 group-hover:w-5 group-hover:h-5 group-hover:border-sky-400/70" />
                <div className="absolute top-0 right-0 w-3 h-3 border-t border-r border-sky-300/80 transition-all duration-300 group-hover:w-5 group-hover:h-5 group-hover:border-sky-400/70" />
                <div className="absolute bottom-0 left-0 w-3 h-3 border-b border-l border-sky-300/80 transition-all duration-300 group-hover:w-5 group-hover:h-5 group-hover:border-sky-400/70" />
                <div className="absolute bottom-0 right-0 w-3 h-3 border-b border-r border-sky-300/80 transition-all duration-300 group-hover:w-5 group-hover:h-5 group-hover:border-sky-400/70" />
                ...
                <div className="flex items-start justify-between mb-5 relative z-10">
                  ...
                  <div className="text-right">
                    <span className="font-mono text-[10px] tracking-widest text-blue-500 block">STEP</span>
                    <span className="font-mono text-2xl font-black text-blue-700/10 group-hover:text-blue-700/20 transition-colors">{step.id}</span>
                  </div>
                </div>

                <div className="relative z-10 flex-1">
                  <h3 className="text-xl font-black tracking-tight text-blue-700 mb-1 group-hover:tracking-normal transition-all duration-300">
                    {step.title}
                  </h3>
                  <p className="font-mono text-[10px] tracking-widest text-blue-500 uppercase mb-3">
                    {step.subtitle}
                  </p>
                ...
                <div className="relative z-10 mt-auto">
                  <div className="h-[1px] bg-gradient-to-r from-blue-300/60 to-transparent mb-4" />

                  <div className="flex items-center justify-between">
                    {step.metrics.map((metric, mi) => (
                      <div key={mi} className="text-left">
                        <span className="font-mono text-[9px] tracking-widest text-blue-500 block">{metric.label}</span>
                        <span className="font-mono text-xs font-bold text-blue-700/70 group-hover:text-blue-700 transition-colors">{metric.value}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center gap-2 mt-4">
                    ...
                    <span className="font-mono text-[9px] tracking-widest text-blue-500">{step.status}</span>
                  </div>
                </div>

                <svg className="absolute bottom-0 right-0 w-24 h-24 text-blue-700/[0.02] group-hover:text-blue-700/[0.05] transition-colors duration-500" viewBox="0 0 100 100">
              ...
              {index < steps.length - 1 && (
                <div className="hidden lg:flex absolute -right-2 top-1/2 -translate-y-1/2 z-20 items-center">
                  <svg width="16" height="16" viewBox="0 0 16 16" className="text-blue-700/20">
        ...
        >
          <div className="inline-flex items-center gap-6 px-8 py-4 border border-sky-200/65 bg-sky-50/70/50">
            <div className="absolute top-0 left-0 w-2 h-2 border-t border-l border-sky-200/65" />
            <div className="absolute bottom-0 right-0 w-2 h-2 border-b border-r border-sky-200/65" />
            {[
              ...
              <div key={i} className="text-center px-4">
                <span className="font-mono text-[9px] tracking-widest text-blue-500 block">{stat.label}</span>
                <span className="font-mono text-sm font-bold text-blue-700">{stat.value}</span>
              </div>
            ))}
          </div>
        </motion.div>
